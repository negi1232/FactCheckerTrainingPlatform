...
contract Quiz_Dapp is class_room {
    ...
    function create_quiz(string memory _title, string memory _explanation, string memory _thumbnail_url, string memory _content, uint _answer_type, string memory _answer_data, string memory _answer, uint _startline_after_epoch, uint _timelimit_after_epoch, uint _reward, uint _respondent_limit) public returns (uint id) {
        ...
        require(token.transferFrom_explanation(msg.sender, address(this), _reward * _respondent_limit, "create_quiz"), "");
        return id;
    }

    event Edit_quiz(address indexed _sender, uint indexed id);

    function edit_quiz(uint id, address owner, string memory _title, string memory _explanation, string memory _thumbnail_url, string memory _content, uint _startline_after_epoch, uint _timelimit_after_epoch) public returns (uint quiz_id) {
        // quizs.push(Quiz(id,msg.sender,_title,_thumbnail_url,_content,_choices,answer_hash,answer_hash,block.timestamp,_reward,_respondent_limit,Answer(msg.sender,block.timestamp,0)));
    ...
    function investment_to_quiz(uint id, uint amount, bool isNotPayingOut, uint numOfStudent) public returns (uint quiz_id) {
        ...
        require(token.transferFrom_explanation(msg.sender, address(this), amount * numOfStudent, "investment_to_quiz"), "");
        return id;
    }

    function get_quiz_all_data(uint _quiz_id) public view returns (uint id, address owner, string memory title, string memory explanation, string memory thumbnail_url, string memory content, uint _answer_type, string memory answer_data, uint start_time_epoch, uint time_limit_epoch, uint reward, uint respondent_count, uint respondent_limit, uint state) {
        id = _quiz_id;
    ...
    function get_quiz(uint _quiz_id) public view returns (uint id, address owner, string memory title, string memory explanation, string memory thumbnail_url, string memory content, string memory answer_data, uint create_time_epoch, uint start_time_epoch, uint time_limit_epoch, uint reward, uint respondent_count, uint respondent_limit) {
        id = _quiz_id;
        owner = quizs[_quiz_id].owner;
        title = quizs[_quiz_id].title;
        explanation = quizs[_quiz_id].explanation;
    ...
    function get_quiz_simple(uint _quiz_id) public view returns (uint id, address owner, string memory title, string memory explanation, string memory thumbnail_url, uint start_time_epoch, uint time_limit_epoch, uint reward, uint respondent_count, uint respondent_limit, uint state) {
        id = _quiz_id;
        owner = quizs[_quiz_id].owner;
        title = quizs[_quiz_id].title;
        explanation = quizs[_quiz_id].explanation;
